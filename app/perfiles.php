...
<body>
  ...
  <div class="content-container">
    ...
    <h2>Detalles</h2>
    <p><strong>Inicio:</strong> [Fecha de inicio]</p>
  </div>
  ...
  <script>
    // Datos de ejemplo (puedes reemplazarlos con datos reales desde una base de datos)
    const pacientes = [
      { id: 1, nombre: "Juan Pérez", diagnostico: "Hipertensión" },
      { id: 2, nombre: "María López", diagnostico: "Diabetes" },
    ];
      ...
      pacientes.forEach((paciente) => {
        ...
        row.innerHTML = `
          ...
          <td class="action-buttons">
            <button class="btn btn-primary btn-sm" onclick="editarPaciente(${paciente.id})">Editar</button>
            <button class="btn btn-danger btn-sm" onclick="eliminarPaciente(${paciente.id})">Eliminar</button>
            <button class="btn btn-success btn-sm" onclick="imprimirDiagnostico(${paciente.id})">Imprimir</button>
          </td>
        `;
    ...
    function eliminarPaciente(id) {
      ...
      if (index !== -1) {
        pacientes.splice(index, 1); // Eliminar el paciente del array
    ...
    function editarPaciente(id) {
      ...
      if (paciente) {
        const nuevoNombre = prompt("Ingrese el nuevo nombre:", paciente.nombre);
        const nuevoDiagnostico = prompt("Ingrese el nuevo diagnóstico:", paciente.diagnostico);
        if (nuevoNombre && nuevoDiagnostico) {
          paciente.nombre = nuevoNombre;
          paciente.diagnostico = nuevoDiagnostico;
          renderTable(); // Volver a renderizar la tabla
          alert(`Paciente con ID ${id} actualizado.`);
        }
      }
    }

    // Función para imprimir el diagnóstico
    function imprimirDiagnostico(id) {
      ...
      if (paciente) {
        ...
        `;
        alert(`Imprimiendo diagnóstico:\n${contenido}`);
        // Aquí podrías implementar la lógica para imprimir en PDF o en una nueva ventana
      }
    }
